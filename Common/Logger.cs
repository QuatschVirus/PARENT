...
namespace PARENT.Common
{
    public class Logger
    {
        public Logger(string path, bool showInfo = true, bool colorOutput = true, ANSICode infoColor = null, bool logToConsole = true)
        {
            LogFile = path;
            ShowInfo = showInfo;
        ...
        public readonly bool LogConsole;
        ...
        public void RawWrite(string content, ANSICode color = null, bool writeToErrorStream = false, bool? writeToConsoleOverride = null, bool? writeToFileOverride = null)
        {
            color ??= ANSICode.Reset;

            if (writeToConsoleOverride ?? LogConsole)
            {
                if (writeToErrorStream)
                {
                    Console.Error.WriteLine(ANSICode.Reset + color + content + ANSICode.Reset);
                } else
                {
                    Console.WriteLine(ANSICode.Reset + color + content + ANSICode.Reset);
                }
            }

            if (writeToFileOverride ?? (LogFile != null))
            {
